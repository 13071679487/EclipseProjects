...
$("#btn_submit").on("click",function(){
	...
	console.log("accout:"+accountVal+"--password:"+passwordVal+"---verifyCode:"+verifycodeVal);
	$(this).parent().children("span").text("");
	$.ajax({
		url:"${path }/checkLogin",
		type:"GET",
		data:{"rname":accountVal,"password":passwordVal,"code":verifycodeVal},
		success:function(result){
			if(result.code==100){
				location.href ="${path }/"+result.extend.href;
			}else{
				$("#verifycode").parent().addClass("has-error");
				$("#verifycode").parents(".input-group").children("span").text(result.extend.errorMsg);
			}
		},
		error:function(){
