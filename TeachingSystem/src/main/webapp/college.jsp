...
<div class="container-fluid">
		<div class="row">
			...
			<div class="col-md-1" style="padding:0;margin:0;text-align: right;width:100px;">
				<select class="form-control" style="padding:0 10px;">
					<option value="1">院系代号</option>
					<option value="2">院系名称</option>
				</select>
			</div>
		...
		<div class="row" style="min-height: 500px;">
			<div class="col-md-12">
				<table class="table table-hover table-condensed table-striped" id="colleges_table">
					<thead>
						<tr>
							<th>
      ...
      <div class="modal-body">
        <form class="form-horizontal">
		  	 <div class="form-group">
			    ...
			    <div class="col-sm-6">
			      <input type="text" name="colid"class="form-control" id="input_colid">
			    </div>
			  </div>
			  <!-- 姓名 -->
			  <div class="form-group">
			    ...
			    <div class="col-sm-6">
			      <input type="text" name="colname"class="form-control" id="input_colname">
			    </div>
			  </div>
		</form>
      </div>
      ...
      <div class="modal-body">
        <form class="form-horizontal">
       		<div class="form-group">
			    <label class="col-sm-2 control-label">院系代号</label>
			    <div class="col-sm-6">
			      <input type="text" name="colid"class="form-control" id="colid">
			    </div>
			  </div>
			  <!-- 姓名 -->
			  <div class="form-group">
			    <label class="col-sm-2 control-label">院系名称</label>
			    <div class="col-sm-6">
			      <input type="text" name="colname"class="form-control" id="colname">
			    </div>
			  </div>
		</form>
      </div>
...
$(document).on("click",".btn_update",function(){
	...
	getCollege($(this).attr("id"));
	 ...
	 //吧员工的id传递过去
	$("#btn_update_college").attr("colid",$(this).attr("id"));
...
function getCollege(id){
	$.ajax({
		url:"${path}/getCollegeById/"+id,
		type:"GET",
		success:function(result){
			if(result.code==100){
		       
				var college=result.extend.college;
				 console.log("请求成功："+college)
				 //console.log($("#collegeUpdateModal").children("#stuid"));
				 $("#colid").val(isEmpty(college.colid)?'':college.colid);
				 $("#colname").val(isEmpty(college.colname)?'':college.colname);
				 console.log("----：8")
				 $("#btn_update_college").attr("colid",college.colid);
				 // $("#updatecollegeModal #sex").val(college.phone);
...
//点击更新按钮，修改学生信息
$("#btn_update_college").on("click",function(){
	var colid=$(this).attr("colid");
	console.log($("#collegeUpdateModal form"));
	$.ajax({
		url:"${path}/updateCollege/"+colid,
		type:'POST',
		data:$("#collegeUpdateModal form").serialize()+"",
		success:function(result){
			if(result.code==100){
				$("#collegeUpdateModal").modal('hide');
				to_page(currentPage);
			}
		}
	})
...
$("#btn_deleteCollege").click(function(){
	...
	if(length>0){
		...
		$.each($(".check_item:checked"),function(data){
			...
			names+="\t";
			ids+=$(this).parents("tr").attr("colid")+"-";
		})
		//去除最后一个“-”
...
$(document).on("click",".btn_delete",function(){
     //1.弹出是否确认删除对话框
     var colname=$(this).parents("tr").find("td:eq(2)").text();
     var colid=$(this).parents("tr").find("td:eq(1)").text();
     //alert($(this).parents("tr").find("td:eq(1)").text());
     alert(colid);
     if(confirm("确认删除学院【"+colname+"】吗？")){
    	 //确认，发送ajax请求删除
    	 $.ajax({
    		 url:"${path}/deleteCollegeById/"+colid,
    		 type:"GET",
    		 success:function(result){
    			 alert(result.msg);
    			 //回到当前页
