...
public class ParallelGateWay {
	...
	public void testDeploy() {
		Deployment deployment = pe.getRepositoryService() // 获取到service
				...
				.addClasspathResource("diagrams/StudentLeaveProcessWithControl3.bpmn") // 加载静态资源
																	// .bpmn文件
				.addClasspathResource("diagrams/StudentLeaveProcessWithControl3.png") // 加载静态资源
																	// .bpmn文件对应的.png文件
				.name("学生请假流程并行网关") // 设置流程名称
				.deploy();
	...
	public void testFindProcess() {
		List<Task> tasks = pe.getTaskService().createTaskQuery().processDefinitionName("校长审批") // 根据委派人来查找任务
				.list();
		for (Task t : tasks) {
