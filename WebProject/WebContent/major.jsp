      ...
      <div class="modal-body">
        <form class="form-horizontal">
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-6">
			      <input type="text" class="form-control" id="input_majorname">
			    </div>
			  </div>
		  ...
		  <div class="form-group">
		    ...
		    <div class="col-sm-6">
		      <input type="text" class="form-control" id="input_majorGender">
		    </div>
		  </div>
...
function build_majors_table(result){
	...
	$.each(majorList,function(index,item){
		...
		var accountTd=$("<td></td>").append(item.majorAccount);
		var nameTd=$("<td></td>").append(item.majorName);
		var genderTd=$("<td></td>").append(item.majorGender=='m'?'男':'女');
		var createTimeTd=$("<td></td>").append(formatDate(item.majorCreatetime));
...
$(document).on("click",".btn_update",function(){
	...
	//根据id查询员工信息
	getMajor($(this).attr("id"));
		
	 //吧员工的id传递过去
	$("#btn_update_major").attr("majorId",$(this).attr("id"));
...
$("#btn_deleteMajor").click(function(){
	...
	if(length>0){
		...
		$.each($(".check_item:checked"),function(data){
			...
			names+="\t";
			ids+=$(this).parents("tr").attr("majorid")+"-";
		})
		//去除最后一个“-”
